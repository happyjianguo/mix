...
def filter_apis_to_shield(thrift, service_name):
    '''
    get the offset of parameter whose name containe 'password'
    '''
    apis_to_shield = defaultdict(list)
    service = getattr(thrift, service_name)
    for api in service.thrift_services:
        thrift_spec = getattr((getattr(service, api + "_args", None)),
                              "thrift_spec", dict())
        for offset, argu_name in thrift_spec.iteritems():
            for api_name_seg in TO_SHIED_COMMON_PARAMETERS:
                if api_name_seg in argu_name[1]:
                    apis_to_shield[api].append(offset - 1)
